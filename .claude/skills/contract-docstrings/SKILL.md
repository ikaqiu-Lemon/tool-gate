...
description: Add clear explanatory comments to Python functions while also writing docstrings that document each function's contract. Identifies input invariants, errors raised on violation, errors from external state, and silenced errors from callees.
argument-hint: "[file, directory, or description of what to focus on]"
disable-model-invocation: true
---

# Contract Docstrings + Explanatory Comments

You are now in contract-docstring mode. Your job is to read Python source files,
analyse each function's actual behaviour, add **ordinary explanatory comments**
where the logic is non-obvious, and write docstrings that document the
...
The added comments serve a different purpose: they should help a reader quickly
understand the code's intent, the meaning of tricky branches, the role of key
data transformations, and why a block exists. Do not restate obvious syntax.
...
- Skip trivial functions (one-liners, simple property accessors, `__repr__`,
  etc.) unless they have non-obvious failure modes or contain logic that would
  genuinely benefit from explanatory comments.
...
3. **Plan the comments.** Identify the smallest set of comments that would make
   the function easier to understand. Prefer comments that explain intent,
   assumptions, tricky branches, non-obvious data reshaping, and side effects.
   Do not add comments for every line.

4. **Write comments and docstrings.** Add concise explanatory comments where
   they materially improve readability. Add or replace docstrings using the
   format described below. Preserve any existing summary line if it's accurate
   — your job is to add contract information, not to rewrite descriptions of
   what the function does.

5. **Verify.** After editing, run the project's test suite and type checker if
   configured. Your edits are comments and docstrings only — they shouldn't
   break anything, but confirm.
...
- **Prefer intent over narration.** Explain why a step exists or what guarantee
  a block is trying to maintain, not what each line literally does.
- **Comment only where useful.** Focus on non-obvious logic, important
  assumptions, delicate conditionals, and side effects.
- **Stay concise.** Keep comments short, direct, and local to the code they
  explain.
- **Use the file's native style.** Match the surrounding comment style,
  indentation, and tone.
- **Do not over-comment.** Avoid comments like "increment counter" or
  "iterate over list" unless the surrounding context makes the step surprising.
- **Do not hide uncertainty.** If a block's purpose is unclear after reading the
  relevant context, ask rather than inventing an explanation.
...
def process_batch(items: list[Record], batch_size: int = 100) -> list[Result]:
    ...
    # Reject empty input early because the batching logic assumes there is at
    # least one record to aggregate.
    if not items:
        raise ValueError("items must not be empty")

    # Normalize into fixed-size chunks so downstream workers can rely on a
    # bounded batch shape.
    ...
```
...
- **Keep comments separate from docstrings.** Use comments for local intent and
  control-flow explanation; use docstrings for function-level contract.
...
Such functions may still benefit from a brief explanatory comment if they sit
inside a larger block of non-obvious logic, but do not force comments where the
...
- **Not a comment flooder.** Don't add comments to every line or paraphrase
  obvious Python syntax. Add only the comments that improve human understanding.

## Presenting Changes

For each file you modify, write a short summary like:

> **`pipeline.py`** — Added explanatory comments and contract docstrings to 4
> functions. `process_batch()` has 3 preconditions (non-empty items, positive
> batch_size, non-None record IDs), silences OSError on cache writes, and now
> has comments explaining the batching invariant and result aggregation flow.
> `connect()` raises on missing env vars but silences DNS resolution failures.
> `load_config()` silently returns defaults for all missing keys — flagged as
...
- **Accuracy over completeness.** If you're unsure whether an invariant is real,
  ...
  than a missing one. A wrong explanatory comment is also worse than no comment.
- **Don't change behaviour.** You are writing comments and docstrings, not
  refactoring code. Don't add assertions, validation, or error handling — even
  if the analysis reveals that some would be useful. The only files you edit are
  the ones that need comments or docstrings.
...
- **Keep comments sparse and high-value.** Prefer one precise comment for a
  tricky block over five comments that simply narrate the code.
