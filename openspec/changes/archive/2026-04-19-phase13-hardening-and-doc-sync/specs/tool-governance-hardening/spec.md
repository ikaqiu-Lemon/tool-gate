# Spec Delta — tool-governance-hardening

> Repair-delta only. This spec does **not** redefine the core governance chain or
> overall architecture. It adds narrow normative requirements that close the
> Phase 1–3 review drifts D1–D4, D6, D7 and records D5 / D8 as doc-sync notes.
>
> **Not in this round**: Phase 4 backlog, any change to the
> `index → policy → grant → rewrite → gate` core chain, any change to overall
> architecture. **Goal of this round**: hardening + consistency cleanup + doc sync.
...
## Doc-Sync Notes (no runtime requirement this round)

These are recorded here so they are visible in the spec delta, but they do
**not** introduce normative requirements in this round. They are discharged by
the doc-sync step defined in the proposal.

- **D5** — `PromptComposer` / `ToolRewriter` constructor signatures in
  `docs/technical_design.md` (+ Chinese mirror) are to be corrected to match
  the current implementation. No code change, no spec change.
- **D8** — `state.active_grants` key semantics (grant_id vs skill_id) are to
  be documented as the current invariant in `docs/technical_design.md` (+
  Chinese mirror) and in the relevant docstrings. Re-keying is explicitly
  deferred.

## Drift Resolution Matrix

| ID  | Category   | Severity | blocking_current_phase | Resolution |
|-----|------------|----------|------------------------|------------|
| D1  | fix now    | P1       | yes                    | fix        |
| D2  | fix now    | P0       | yes                    | fix        |
| D3  | fix now    | P1       | yes                    | fix        |
| D4  | fix now    | P1       | yes                    | fix (tests)|
| D5  | doc sync   | P2       | no                     | doc-sync   |
| D6  | fix now    | P1       | yes                    | fix        |
| D7  | fix now    | P1       | yes                    | fix        |
| D8  | doc sync   | P2       | no                     | doc-sync   |
| D9  | backlog    | P2       | no                     | defer      |
| D10 | backlog    | P2       | no                     | defer      |

Resolution order for `fix now` items is fixed by the proposal:
**D2 → D1 → D6 → D3 → D7 → D4**.
